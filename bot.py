...
ADMIN_TELEGRAM_IDS = {
    item.strip()
    for item in os.getenv("ADMIN_TELEGRAM_IDS", "").replace(";", ",").split(",")
    if item.strip()
}
...
def is_restart_allowed(chat_id: int) -> bool:
    """Если ADMIN_TELEGRAM_IDS не задан, рестарт доступен всем пользователям бота.
    Если задан — только указанным Telegram ID через запятую.
    """
    return not ADMIN_TELEGRAM_IDS or str(chat_id) in ADMIN_TELEGRAM_IDS
...
def restart_bot(chat_id: int) -> None:
    if not is_restart_allowed(chat_id):
        bot.send_message(
            chat_id,
            "⛔ Рестарт доступен только администратору бота. Добавь свой Telegram ID в Railway Variable ADMIN_TELEGRAM_IDS.",
            reply_markup=main_keyboard(chat_id),
        )
        return
...
def start(message):
    bot.send_message(
        ...
        f"✅ Кнопка 🏓 Ping показывает задержку ответа и версию {BOT_VERSION}.\n"
        "✅ Кнопка ♻️ Рестарт перезапускает процесс бота.\n\n"
...
def help_cmd(message):
    bot.send_message(
        ...
        f"• 🏓 Ping — показать ping и версию {BOT_VERSION}\n"
        "• ♻️ Рестарт — перезапустить процесс бота\n"
...
def handle(message):
    ...
    if text in {"♻️ Рестарт", "рестарт", "restart", "/restart"}:
        return restart_bot(message.chat.id)
